...
func (c *Conn) ExecutePrepare(command string, args ...interface{}) (*mysql.Result, error) {
...
// Result Set Header:
//   1-9 Field count
//   1-9 可选字段，一般不出现(只有像SHOW COLUMNS这种语句的执行结果才会用到额外信息标识表格的列数量)
// Field 列信息
// EOF
// Row Data 行信息
// EOF
...
//EOF结构用于标识Field和Row Data的结束
//在预处理语句中，EOF也被用来标识参数的结束
...
// 受影响行数(1-9): 当执行INSERT/UPDATE/DELETE语句时所影响的数据行数
// 索引ID值(1-9):该值为AUTO_INCREMENT索引字段生成，如果没有索引字段，则为0x00
