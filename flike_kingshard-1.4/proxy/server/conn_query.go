...
func (c *ClientConn) handleQuery(sql string) (err error) {
	...
	// 分片预处理
	// hasHandled - true  已经处理完成，未经过分片
	// hasHandled - false 需要进行分片操作
	...
	fmt.Printf("MDP need shard!!\n")
	var stmt sqlparser.Statement
	// 解析sql语句,通过yacc语法分析规则,得到的stmt是一个interface
...
func (c *ClientConn) executeInNodePrepare(conn *backend.BackendConn, sql string, args []interface{}) ([]*mysql.Result, error) {
	var state string
	startTime := time.Now().UnixNano()
	r, err := conn.ExecutePrepare(sql, args...)
	if err != nil {
		state = "ERROR"
	} else {
		state = "OK"
...
func (c *ClientConn) executeInMultiNodes(conns map[string]*backend.BackendConn, sqls map[string][]string, args []interface{}) ([]*mysql.Result, error) {
	...
	offsert := 0
	//每条sql经过分片规则，可能会分担到多个node节点中
	for nodeName, co := range conns {
		//找到对应node的sql
		s := sqls[nodeName] //[]string, s为索引
