...
type Stmt struct {
	...
	args []interface{} //存放具体的Prepare stmt数据
...
func (c *ClientConn) handleStmtPrepare(sql string) error {
	...
	fmt.Printf("MDP handleStmtPrepare: %+v\n", s)
...
func (c *ClientConn) handleStmtExecute(data []byte) error {
	...
	if paramNum > 0 {
		...
		pos += nullBitmapLen
		//fmt.Printf("MDP after nullBitmapLen nullBitmaps:%v, nullBitmapLen:%d pos: %d\n",nullBitmaps, nullBitmapLen, pos)
		...
		if data[pos] == 1 {
			...
			fmt.Printf("MDP paramValues: %v", paramValues)
		}

		// nullBitmaps [0]
		// paramTypes [8 0]
		// paramValues [1 0 0 0 0 0 0 0]
...
func (c *ClientConn) handlePrepareSelect(stmt *sqlparser.Select, sql string, args []interface{}) error {
	...
	rs, err = c.executeInNodePrepare(conn, sql, args)
	if err != nil {
...
// nullBitmaps [0]
// paramTypes [8 0]  参数类型 是否无符号
// paramValues [1 0 0 0 0 0 0 0] 参数值1, 8个字节
func (c *ClientConn) bindStmtArgs(s *Stmt, nullBitmap, paramTypes, paramValues []byte) error {
	...
	// s.params 为 Prepare()从server中返回的参数个数
	for i := 0; i < s.params; i++ {
		//依次取出每一位
