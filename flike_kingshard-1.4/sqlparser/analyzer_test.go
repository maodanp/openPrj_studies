...
//SELECT user_id, u.name, SUM(hangup_time), SUM(credit_point), SUM(task_num)  FROM client_statistics_shard AS m JOIN alliance_user AS u ON m.user_id = u.id GROUP BY user_id LIMIT 10;
