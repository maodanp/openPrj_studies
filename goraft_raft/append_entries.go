...
type AppendEntriesRequest struct {
	Term         uint64               //Leader的任期数
	PrevLogIndex uint64               //新日志项的前一个日志编号
	PrevLogTerm  uint64               //新日志项的前一个日志任期
	CommitIndex  uint64               //Leader 节点的commitIndex
	LeaderName   string               //Leader标识
	Entries      []*protobuf.LogEntry //日志项(为空则作为心跳，出于性能考虑可能一次发送多项)
}
