...
func (l *Log) open(path string) error {
	...
	debugf("open.log.append startIndex[%d] commitIndex[%d]", l.startIndex, l.commitIndex)
	// Read the file and decode entries.
	for {
		// Instantiate log entry and decode into it.
		// entry的 index/item/commandName/command从日志中读取
		...
		if entry.Index() > l.startIndex {
			// Append entry. 放入 log entry列表
			l.entries = append(l.entries, entry)
			//对于小于提交index的则全部进行提交一遍
...
func (l *Log) getEntriesAfter(index uint64, maxLogEntriesPerRequest uint64) ([]*LogEntry, uint64) {
	...
	// 如果index在start前，则返回nil, 此时需要发送快照
...
/*
   更新commit index
   将旧的commit index 至 index之间的entry都提交[ 调用 l.ApplyFunc(entry, command) ]
*/
func (l *Log) setCommitIndex(index uint64) error {
	...
	// 新的leader的log entries必须比大多数后选择的新；而不是提交的索引比其他后选择新
	...
	for i := l.commitIndex + 1; i <= index; i++ {
		...
		// 将需要提交的log entry应用到状态机
		returnValue, err := l.ApplyFunc(entry, command)

		debugf("path:%s setCommitIndex.set.result index: %v, entries index: %v", l.path, i, entryIndex)
		if entry.event != nil {
...
/*
截断未提交的entry, index >= commitIndex, 截断点必须从 commitIndex起始
*/
func (l *Log) truncate(index uint64, term uint64) error {
	l.mutex.Lock()
	defer l.mutex.Unlock()
	debugln("log.truncate: ", index, term)
...
func (l *Log) appendEntry(entry *LogEntry) error {
	...
	//fmt.Printf("appendEntry file[%s]", l.file.Name())
	debugf("appendEntry file[%s] ", l.file.Name())
	...
	// Write to storage. 将entry信息写入 l.file
	/*
	   pb:  Index:1 Term:2 CommandName:"raft:join" Command:"{\"name\":\"localhost:1000\",\"connectionString\":\"\"}\n
	        经过proto的编码、放入文件
	*/
	debugf("appendEntry pb[%s]", entry.pb)
