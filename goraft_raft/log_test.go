...
func TestLogExistingLog(t *testing.T) {
	...
	/*
	   将log netry编码后存入log
	   log.open(f.Name()) --  Read the file and decode entries
	*/
...
// 从日志中将log entry恢复至内存的log entries队列
...
func TestLogTruncate(t *testing.T) {
	...
	// Truncate committed entry. 截断提交后的entry
	...
	// Truncate entry with mismatched term. 截断错误term的entry
