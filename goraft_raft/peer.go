...
type Peer struct {
	...
	prevLogIndex      uint64
	stopChan          chan bool     //remove
	heartbeatInterval time.Duration //心跳时间间隔
	lastActivity      time.Time     //最后激活时间
	sync.RWMutex
}
...
func (p *Peer) heartbeat(c chan bool) {
	...
	debugf("peer.heartbeat: %s[%d]", p.Name, p.heartbeatInterval)
...
func (p *Peer) flush() {
	...
	debugf("peer:%s getEntriesAfter[preLogIdx:%d] len(entries): %d", p.Name, prevLogIndex, len(entries))
	if entries != nil {
