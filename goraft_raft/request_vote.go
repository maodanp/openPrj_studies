...
type RequestVoteRequest struct {
	peer          *Peer  //所有peer节点
	Term          uint64 //candidate任期
	LastLogIndex  uint64 //candidate最新日志项编号
	LastLogTerm   uint64 //candidate最新日志项任期
	CandidateName string //candidate标识
}
...
type RequestVoteResponse struct {
	peer        *Peer
	Term        uint64 //当前任期
	VoteGranted bool   //是否投票给candidate， true表示投票
}
