...
type server struct {
	*eventDispatcher
	...
	c                 chan *ev
	electionTimeout   time.Duration
	heartbeatInterval time.Duration //给peer的心跳间隔
...
func NewServer(name string, path string, transporter Transporter, stateMachine StateMachine, ctx interface{}, connectionString string) (Server, error) {
	...
	s.log.ApplyFunc = func(e *LogEntry, c Command) (interface{}, error) {
		...
		// 分发 commit 事件，其中值为 log entry。
		//s.debugln("server.ApplyFunc")
...
/*
   如果 log entries存在，并且没有收到Append Entries回复，则转变为 candidate
   如果 log entries不存在，则等待其他节点的Append Entries
   如果 log entries不存在，并且收到了 self-join command，则将自己转变成leader，并提交entry
*/
	...
	for s.State() == Follower {
		...
		case e := <-s.c:
			...
			case JoinCommand:
				...
				//收到self-join命令，并且log entries为空，则立即将状态变为 leader
				...
				s.debugln("server.followerLoop", s.State())
				/*
				   AppendEntries RPC是由Leader节点发送给其他节点:
				   1. 当其entries域为空时，该RPC作为Leader的心跳
				   2. 当entries域不为空时，将leader的entries域中的log添加到自己的entries中
				*/
			case *AppendEntriesRequest:
				...
				//对于AppendEntries请求的处理
				e.returnValue, update = s.processAppendEntriesRequest(req)

				//由candidate发送给其他节点，请求其他节点为自己投
		...
		// update 表示需要重新更新超时时间，否则满足以下情况不更新:
		//  1. 接受到了有效的AEs RPC
		//  2. 选票投给了candidate
		if update {
			since = time.Now()
...
func (s *server) leaderLoop() {
	...
	//当选为Leader后，立即向其他节点发送 AppendEntries RPC，并且每隔一段时间重复发送心跳RPC，防止其他节点选举超时
	s.routineGroup.Add(1)
	go func() {
		defer s.routineGroup.Done()
		//这里模拟客户端向leader发送NOP、leader再通过channel收到该command后分发给follower
		s.Do(NOPCommand{})
	}()
	...
	for s.State() == Leader {
		...
		case e := <-s.c:
			switch req := e.target.(type) {
			//收到客户端请求，主要负责创建log entry，放入leader的entries域中
			//循环等待
			case Command:
				s.processCommand(req, e)
				continue
...
func (s *server) processCommand(command Command, e *ev) {
	...
	//fmt.Printf("create entry %+v\n", *entry)
	//表示leader自身的log 已经append成功
	s.syncedPeer[s.Name()] = true
	if len(s.peers) == 0 {
		//如果没有peers,为单节点，则每隔command都会被提交
		commitIndex := s.log.currentIndex()
		s.debugln("processCommand commit index ", commitIndex)
...
func (s *server) processAppendEntriesRequest(req *AppendEntriesRequest) (*AppendEntriesResponse, bool) {
	s.debugln("server.ae.process")

	//如果AppendEntries请求的term小于 该server当前term, 返回失败
	if req.Term < s.currentTerm {
		s.debugln("server.ae.error: stale term")
		//term success index commitIndex
	...
	// 将req.PrevLogIndex 编号后的log entry都截断了
...
// 发出投票请求, 能够得到回应情形：
// 1. 该vote的term与某server的当前term相同，且某server没投票过
// 2. 该vote的term比某server的当前term大
...
func (s *server) processRequestVoteRequest(req *RequestVoteRequest) (*RequestVoteResponse, bool) {
	...
	// 如果term相等，且我们已经投给了不同的candidate(votedFor参数),则不投给新的candidate
	if req.Term > s.Term() {
...
func (s *server) processSnapshotRecoveryRequest(req *SnapshotRecoveryRequest) *SnapshotRecoveryResponse {
	...
	fmt.Printf("server.processSnapshotRecoveryRequest")
...
func (s *server) LoadSnapshot() error {
	fmt.Printf("server.LoadSnapshot")
...
/*
{"commitIndex":4,"peers":[{"name":"1","connectionString":""},{"name":"2","connectionString":""}]}
*/
...
func (s *server) debugln(v ...interface{}) {
	...
	fmt.Printf("[%s Term:%d] %s", s.name, s.Term(), fmt.Sprintln(v...))
}
...
func (s *server) traceln(v ...interface{}) {
	...
	//fmt.Printf("[%s] %s", s.name, fmt.Sprintln(v...))
}
