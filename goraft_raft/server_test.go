...
func TestServerRequestVote1(t *testing.T) {
	...
	defer server.Stop()
	//收到节点“foo”(term:1 lastLogIndex:1 lastLogTerm:0) 的投票
	//则节点“1”转变为 follower
...
func TestServerRequestVoteDenyIfCandidateLogIsBehind(t *testing.T) {
	...
	// lastIndex/lastLogTerm不一致
	// term更新、但不给candidate投票
...
func TestServerPromoteSelf(t *testing.T) {
	...
	//2倍的超时时间，则follower自动升级为candidate
...
func TestServerAppendEntries(t *testing.T) {
	...
	/* term:1
		   prevLogIndex:1 新日志项的前一个日志编号
		   prevLogTerm: 1 新日志项的前一个任期号
		   commitIndex: 1 leader节点的commit Index
	       entries:      新添加的信息
	*/
...
func TestServerAppendEntriesOverwritesUncommittedEntries(t *testing.T) {
	...
	/* term:2
		   prevLogIndex:1 新日志项的前一个日志编号
		   prevLogTerm: 1 新日志项的前一个任期号
		   commitIndex: 2 leader节点的commit Index
	       entries:      新添加的信息
	*/
	// index:2 处的未提交，所以能够被覆盖
	...
	for i, entry := range s.(*server).log.entries {
		if entry.Term() != entries[i].Term() || entry.Index() != entries[i].Index() || !bytes.Equal(entry.Command(), entries[i].Command()) {
			//t.Fatalf("AppendEntries failed: %d/%v", resp.Term, resp.Success)
		}
	}
}
...
func TestServerRecoverFromPreviousLogAndConf(t *testing.T) {
	...
	for _, name := range names {
		s := servers[name]
		//1(NopCommand)+3(DefaultJoinCommand)+1(testCommand2)
		if s.CommitIndex() != 5 {
			t.Fatalf("%s commitIndex is invalid [%d/%d]", name, s.CommitIndex(), 6)
		}
		s.Stop()
	}
	...
	for _, name := range names {
		...
		// 仅仅提交到最后一次的join command
		// 因为testCommand2 的Apply()中未调用server.AddPeer() 进行writeConf()
		// 但是log文件中包含了所有entries信息
		if s.CommitIndex() != 4 {
			t.Fatalf("%s recover phase 1 commitIndex is invalid [%d/%d]", name, s.CommitIndex(), 4)
		}

		// peer conf should be recovered
		if len(s.Peers()) != 2 {
			t.Fatalf("%s recover phase 1 peer failed! [%d/%d]", name, len(s.Peers()), 2)
		}
	}

	// let nodes talk to each other
	// 通过transport，互相之间能够通信
	for _, name := range names {
...
func TestServerMultiNode(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		num1 := strconv.Itoa((i+3)%(len(servers)) + 1)
		toStop := servers[num]
		toStop1 := servers[num1]

		// Stop the first server and wait for a re-election.
		time.Sleep(2 * testElectionTimeout)
		debugln("Disconnect ", toStop.Name())
		debugln("disconnect ", num, " ", num1)
		toStop.SetTransporter(disTransporter)
		toStop1.SetTransporter(disTransporter)
		time.Sleep(2 * testElectionTimeout)
		...
		toStop1.SetTransporter(transporter)
	}
